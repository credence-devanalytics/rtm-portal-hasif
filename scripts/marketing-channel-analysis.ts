...
import { marketingChannelByYear, marketingChannelByMonth } from '../drizzle/schema';
...
class MarketingChannelAnalyzer {
  ...
  async analyzeYearlyData(): Promise<void> {
    ...
    try {
      ...
      const reportTypesQuery = await this.db
        .selectDistinct({ reportType: marketingChannelByYear.report_type })
        .from(marketingChannelByYear);

      console.log('\n📊 Data Overview:');
      console.log('Years available:', yearsQuery.map(y => y.year).join(', '));
      console.log('Report types:', reportTypesQuery.map(r => r.reportType).join(', '));
  ...
  private async analyzeReportTypeByYear(reportType: string): Promise<void> {
    const data = await this.db
      .select()
      .from(marketingChannelByYear)
      .where(eq(marketingChannelByYear.report_type, reportType))
      .orderBy(asc(marketingChannelByYear.year), asc(marketingChannelByYear.saluran));
    ...
    Object.keys(yearlyData).sort().forEach(year => {
      ...
      console.log(`  ${year}: RM ${total.toLocaleString()} | ${channels} active channels`);
    });
  ...
  private async analyzeChannelPerformance(): Promise<void> {
    ...
    const channelQuery = await this.db
      .select({
        ...
        totalRevenue: sql<number>`SUM(CAST(${marketingChannelByYear.value} AS DECIMAL))`.mapWith(Number),
        yearCount: sql<number>`COUNT(DISTINCT ${marketingChannelByYear.year})`.mapWith(Number),
        avgRevenue: sql<number>`AVG(CAST(${marketingChannelByYear.value} AS DECIMAL))`.mapWith(Number)
      })
      .from(marketingChannelByYear)
      .groupBy(marketingChannelByYear.saluran)
      .orderBy(desc(sql`SUM(CAST(${marketingChannelByYear.value} AS DECIMAL))`));

    console.log('\nTop 10 Channels by Total Revenue:');
    channelQuery.slice(0, 10).forEach((channel, index) => {
      console.log(`${(index + 1).toString().padStart(2)}. ${channel.saluran.padEnd(20)} | RM ${channel.totalRevenue.toLocaleString()} | Avg: RM ${channel.avgRevenue.toFixed(0)} | Years: ${channel.yearCount}`);
    });
  ...
  private async generateYearlyStatistics(): Promise<void> {
    ...
    for (const year of years) {
      const yearData = await this.db
        .select({
          value: sql<number>`CAST(${marketingChannelByYear.value} AS DECIMAL)`.mapWith(Number)
        })
        .from(marketingChannelByYear)
      ...
      console.log(`  Average Revenue: RM ${stats[year].averageRevenue.toFixed(0)}`);
      console.log(`  Range: RM ${stats[year].minRevenue.toLocaleString()} - RM ${stats[year].maxRevenue.toLocaleString()}`);
    }

    // Calculate year-over-year growth
    if (stats[2022] && stats[2023]) {
      stats[2023].yearOverYearGrowth = ((stats[2023].totalRevenue - stats[2022].totalRevenue) / stats[2022].totalRevenue) * 100;
    }
    if (stats[2023] && stats[2024]) {
      stats[2024].yearOverYearGrowth = ((stats[2024].totalRevenue - stats[2023].totalRevenue) / stats[2023].totalRevenue) * 100;
    }

    console.log('\n📈 Year-over-Year Growth:');
    console.log(`2022-2023: ${stats[2023]?.yearOverYearGrowth.toFixed(1) || 'N/A'}%`);
    console.log(`2023-2024: ${stats[2024]?.yearOverYearGrowth.toFixed(1) || 'N/A'}%`);
  ...
  async analyzeMonthlyData(): Promise<void> {
    ...
    try {
      ...
      console.log('Years available:', yearsQuery.map(y => y.year).join(', '));
      console.log('Months available:', monthsQuery.map(m => m.month).join(', '));
  ...
  private async analyzeMonthlyTrends(): Promise<void> {
    ...
    const monthlyData = await this.db
      .select({
        ...
        totalRevenue: sql<number>`SUM(CAST(${marketingChannelByMonth.value} AS DECIMAL))`.mapWith(Number),
        channelCount: sql<number>`COUNT(DISTINCT ${marketingChannelByMonth.saluran})`.mapWith(Number)
      })
      .from(marketingChannelByMonth)
      .groupBy(marketingChannelByMonth.year, marketingChannelByMonth.month)
      .orderBy(asc(marketingChannelByMonth.year), asc(marketingChannelByMonth.month));
    ...
    Object.keys(yearlyMonthlyData).sort().forEach(year => {
      console.log(`\n${year} Monthly Revenue:`);
      const months = yearlyMonthlyData[parseInt(year)];

      months.forEach(month => {
        console.log(`  ${month.month.padEnd(10)}: RM ${month.totalRevenue.toLocaleString()} | ${month.channelCount} channels`);
      });

      const yearTotal = months.reduce((sum, month) => sum + month.totalRevenue, 0);
      const monthAvg = yearTotal / months.length;
      console.log(`  Year Total: RM ${yearTotal.toLocaleString()} | Monthly Avg: RM ${monthAvg.toFixed(0)}`);
    });
  ...
  private async analyzeSeasonalPatterns(): Promise<void> {
    ...
    const allMonthlyData = await this.db
      .select({
        month: marketingChannelByMonth.month,
        value: sql<number>`CAST(${marketingChannelByMonth.value} AS DECIMAL)`.mapWith(Number),
        year: marketingChannelByMonth.year
      })
      .from(marketingChannelByMonth);
    ...
    monthStats.forEach((stat, index) => {
      console.log(`${(index + 1).toString().padStart(2)}. ${stat.month.padEnd(10)} | Avg: RM ${stat.average.toFixed(0)} | Range: RM ${stat.min.toLocaleString()} - RM ${stat.max.toLocaleString()}`);
    });
  ...
  async analyzeUniqueValues(): Promise<void> {
    ...
    const reportTypes = await this.db
      .selectDistinct({
        reportType: marketingChannelByYear.report_type,
        reportTitle: marketingChannelByYear.report_title
      })
      .from(marketingChannelByYear);
