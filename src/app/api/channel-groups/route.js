...
export async function GET(request) {
	try {
		...
		const summary = await getDb()
			.select({
				totalRecords: count(),
				uniqueGroups: sql`COUNT(DISTINCT ${mentionsClassify.groupname})`.as(
					"uniqueGroups"
				),
				uniqueChannels: sql`COUNT(DISTINCT ${mentionsClassify.channel})`.as(
					"uniqueChannels"
				),
				uniqueChannelGroups:
					sql`COUNT(DISTINCT ${mentionsClassify.channelgroup})`.as(
						"uniqueChannelGroups"
					),
			})
			.from(mentionsClassify)
		...
		const sortedChannelGroups = Object.values(processedData).sort(
		...
		console.log(
			`Channel Groups API: Found ${sqlFormatData.length} unique combinations`
		);
		...
		return NextResponse.json(
			{
				...
				sqlQuery:
					"SELECT groupname, channel, channelgroup, count(*) FROM mentions_classify GROUP BY groupname, channel, channelgroup ORDER BY channelgroup",
			},
			{ status: 500 }
		);
	}
