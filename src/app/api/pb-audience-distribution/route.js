...
export async function GET() {
  try {
    ...
    const totalAllSegments = chartData.reduce((sum, item) => sum + item.totalUsers, 0);
    chartData.forEach(item => {
      item.percentage = totalAllSegments > 0 ? 
        ((item.totalUsers / totalAllSegments) * 100).toFixed(1) : 0;
    });

    // Calculate summary statistics
    const totalSegments = chartData.length;
    const largestSegment = chartData[0] || { audienceName: 'No data', totalUsers: 0 };
    const avgUsersPerSegment = totalSegments > 0 ? 
      Math.round(totalAllSegments / totalSegments) : 0;
