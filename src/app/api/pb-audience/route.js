...
export async function GET() {
  try {
    ...
    const avgDailyUsers = totalRecords > 0 ? 
      Math.round(chartData.reduce((sum, item) => sum + item.totalUsers, 0) / totalRecords) : 0;
    const avgNewUsers = totalRecords > 0 ? 
      Math.round(chartData.reduce((sum, item) => sum + item.newUsers, 0) / totalRecords) : 0;
