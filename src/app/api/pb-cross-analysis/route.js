...
async function getOverviewAnalysis(startDate, endDate) {
  const [audienceData, ageData, genderData, regionData, sourceData] = await Promise.all([
  ...
  const totalUsers = audienceData.reduce((sum, item) => sum + (item.totalUsers || 0), 0);
  const totalNewUsers = audienceData.reduce((sum, item) => sum + (item.newUsers || 0), 0);

  // Top demographics
  const ageGroups = {};
  ageData.forEach(item => {
    ageGroups[item.userAgeBracket] = (ageGroups[item.userAgeBracket] || 0) + item.activeUsers;
  });
  ...
  genderData.forEach(item => {
    genderGroups[item.userGender] = (genderGroups[item.userGender] || 0) + item.activeUsers;
  });

  const regionGroups = {};
  regionData.forEach(item => {
    regionGroups[item.region] = (regionGroups[item.region] || 0) + item.activeUsers;
  });
  const topRegion = Object.entries(regionGroups).reduce((max, [region, count]) => 
    count > max.count ? { region, count } : max, { region: 'N/A', count: 0 });
...
async function getDailyTrendsAnalysis(startDate, endDate) {
  ...
  audienceData.forEach(item => {
    ...
    dailyTrends[date].returningUsers = dailyTrends[date].totalUsers - dailyTrends[date].newUsers;
  });

  const chartData = Object.values(dailyTrends).sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      summary: {
        ...
        avgDailyUsers: chartData.length > 0 ? 
          Math.round(chartData.reduce((sum, item) => sum + item.totalUsers, 0) / chartData.length) : 0,
        avgNewUsers: chartData.length > 0 ? 
          Math.round(chartData.reduce((sum, item) => sum + item.newUsers, 0) / chartData.length) : 0
      },
