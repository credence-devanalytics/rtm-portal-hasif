...
export async function GET() {
	try {
		...
		chartData.forEach((item) => {
			item.percentage =
				totalUsers > 0
					? parseFloat(((item.totalUsers / totalUsers) * 100).toFixed(1))
					: 0;
			item.newUserRate =
				item.totalUsers > 0
					? parseFloat(((item.newUsers / item.totalUsers) * 100).toFixed(1))
					: 0;
		});
		...
		const response = {
			...
			data: {
				...
				summary: {
					...
					totalReturningUsers: chartData.reduce(
					...
					avgNewUserRate:
						chartData.length > 0
							? parseFloat(
									(
										chartData.reduce((sum, item) => sum + item.newUserRate, 0) /
										chartData.length
									).toFixed(1)
							  )
							: 0,
