  ...
  try {
    ...
    const popularPagesData = await db
      .select({
        ...
        totalPageViews: sql`SUM(${pbPopularPages.screenPageViews})`.as('totalPageViews'),
        totalActiveUsers: sql`SUM(${pbPopularPages.activeUsers})`.as('totalActiveUsers'),
        recordCount: sql`COUNT(*)`.as('recordCount'),
        avgPageViews: sql`AVG(${pbPopularPages.screenPageViews})`.as('avgPageViews'),
        avgActiveUsers: sql`AVG(${pbPopularPages.activeUsers})`.as('avgActiveUsers')
      })
      .from(pbPopularPages)
    ...
    const totalPageViews = chartData.reduce((sum, item) => sum + item.totalPageViews, 0);
    chartData.forEach(item => {
      item.viewsPerUser = item.totalActiveUsers > 0 ? 
        parseFloat((item.totalPageViews / item.totalActiveUsers).toFixed(2)) : 0;
      item.percentage = totalPageViews > 0 ? 
        parseFloat(((item.totalPageViews / totalPageViews) * 100).toFixed(1)) : 0;
    });
    ...
    const cleanChartData = chartData.map(item => ({
      ...item,
      cleanPageName: item.pageName ? item.pageName.replace(' - Portal Berita', '') : 'Unknown Page'
    }));
    ...
    const response = {
      ...
      data: {
        ...
        summary: {
          totalPageViews,
          totalActiveUsers: chartData.reduce((sum, item) => sum + item.totalActiveUsers, 0),
          topPage: topPage.pageName ? topPage.pageName.replace(' - Portal Berita', '') : 'N/A',
          topPageViews: topPage.totalPageViews || 0,
          topPagePercentage: topPage.percentage || 0,
          pageCount: chartData.length,
          topPages: topPages.map(p => ({ 
            page: p.pageName ? p.pageName.replace(' - Portal Berita', '') : 'Unknown',
            views: p.totalPageViews, 
            users: p.totalActiveUsers,
            percentage: p.percentage,
            viewsPerUser: p.viewsPerUser
          })),
          avgViewsPerPage: chartData.length > 0 ? 
            Math.round(totalPageViews / chartData.length) : 0,
          avgViewsPerUser: chartData.length > 0 ? 
            parseFloat((chartData.reduce((sum, item) => sum + item.viewsPerUser, 0) / chartData.length).toFixed(2)) : 0
        }
      }
    };
