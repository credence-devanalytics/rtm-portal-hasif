...
import { pbAudienceRegion, pbAudienceRegionGender } from '../../../../drizzle/schema';
  ...
  try {
    ...
    if (analysisType === 'region-gender') {
      ...
      const regionGenderData = await db
        .select({
          ...
          totalActiveUsers: sql`SUM(${pbAudienceRegionGender.activeUsers})`.as('totalActiveUsers'),
          totalNewUsers: sql`SUM(${pbAudienceRegionGender.newUsers})`.as('totalNewUsers'),
          recordCount: sql`COUNT(*)`.as('recordCount')
        })
        .from(pbAudienceRegionGender)
        .groupBy(pbAudienceRegionGender.region, pbAudienceRegionGender.userGender);
    ...
    const regionData = await db
      .select({
        ...
        totalNewUsers: sql`SUM(${pbAudienceRegion.newUsers})`.as('totalNewUsers'),
        recordCount: sql`COUNT(*)`.as('recordCount')
      })
    ...
    const response = {
      ...
      data: {
        ...
        summary: {
          ...
          topRegions: topRegions.map(r => ({ region: r.region, users: r.activeUsers, percentage: r.percentage }))
        },
        analysisType: 'region'
      }
    };
