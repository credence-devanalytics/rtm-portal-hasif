	...
	try {
		...
		try {
			...
			if (sentiment && sentiment !== "all") {
				whereConditions.push(
					sql`LOWER(${
						mentionsClassifyPublic.sentiment
			...
			console.log(
				"❌ Database connection failed, switching to local data:",
				dbError.message
			);
			usingLocalData = true;
			...
			if (sentiment && sentiment !== "all") {
				processedData = processedData.filter(
					(item) => item.sentiment?.toLowerCase() === sentiment.toLowerCase()
				);
			}
			...
			metrics = {
				...
				totalReach: processedData.reduce(
					(sum, item) => sum + (item.reach || 0),
					0
				),
				totalInteractions: processedData.reduce(
					(sum, item) =>
						sum +
						(item.likecount || 0) +
						(item.sharecount || 0) +
				...
				avgEngagement:
					processedData.reduce(
						(sum, item) => sum + (item.engagementrate || 0),
						0
					) / processedData.length,
				avgConfidence:
					processedData.reduce((sum, item) => sum + (item.confidence || 0), 0) /
					processedData.length,
				totalTokens: processedData.reduce(
			...
			sentimentBreakdown = Object.entries(sentimentGroups).map(
				([sentiment, data]) => ({
					sentiment,
					count: data.count,
			...
			const topicGroups: Record<
				string,
				{ count: number; totalConfidence: number; totalReach: number }
			> = {};
			processedData.forEach((item) => {
			...
			topicDistribution = Object.entries(topicGroups)
				.map(([topic, data]) => ({
					topic,
					count: data.count,
					avgConfidence: data.totalConfidence / data.count,
			...
			const platformGroups: Record<
				string,
				{ count: number; totalReach: number }
			> = {};
			processedData.forEach((item) => {
			...
			platformDistribution = Object.entries(platformGroups)
				.map(([platform, data]) => ({
					platform,
					count: data.count,
					totalReach: data.totalReach,
			...
			// Get unique topics
			uniqueTopics = [
				...new Set(processedData.map((item) => item.topic).filter(Boolean)),
			];
			...
			processedData.forEach((item) => {
				...
				dateGroups[date].totalInteractions +=
					Number(item.likecount || 0) +
					Number(item.sharecount || 0) +
					Number(item.commentcount || 0);
			});

			dailySentimentTrends = Object.entries(dateGroups)
				.map(([date, data]) => ({
					date,
					count: data.total,
			...
			confidenceDistribution = Object.entries(confRanges)
				.map(([range, count]) => ({
					range,
					count,
				}))
		...
		const response = {
			...
			metrics: {
				...
				totalInteractions:
					parseInt(String((metrics as any).totalInteractions)) || 0,
			...
			topics: {
				...
				uniqueTopics: uniqueTopics.map
					? uniqueTopics.map((t) => t.topic || t).filter(Boolean)
					: uniqueTopics.filter(Boolean),
			},
		...
		console.log(
			`Public Mentions API: ${
				usingLocalData ? "Using local data" : "Using database"
			} - Processed ${mentions.length} mentions for filters:`,
			{
				days,
				platform: platform || "all",
				sentiment: sentiment || "all",
		...
		if (responseSize > 10 * 1024 * 1024) {
			// 10MB
			console.warn(
				"⚠️ Large response detected:",
				responseSizeMB,
				"MB - Consider reducing limit"
			);
		}
