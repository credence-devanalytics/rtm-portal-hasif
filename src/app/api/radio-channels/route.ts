...
export async function GET() {
  try {
    ...
    const radioChannelData = await db
      ...
      .where(eq(marketingChannelByYear.report_type, 'Table 4'));
    ...
    Object.keys(channelGroups).forEach(groupType => {
      ...
      const channelsWithGrowth = channels.map(channel => {
        ...
        const growth2022to2023 = value2022 > 0 ? ((value2023 - value2022) / value2022 * 100).toFixed(1) : 'N/A';
      ...
      const groupTotal2022 = channelsWithGrowth.reduce((sum, ch) => sum + ch[2022], 0);
      const groupTotal2023 = channelsWithGrowth.reduce((sum, ch) => sum + ch[2023], 0);
      const groupTotal2024 = channelsWithGrowth.reduce((sum, ch) => sum + ch[2024], 0);
      const groupTotalGrowth2022to2023 = groupTotal2022 > 0 ? ((groupTotal2023 - groupTotal2022) / groupTotal2022 * 100).toFixed(1) : 'N/A';
      const groupTotalGrowth2023to2024 = groupTotal2023 > 0 ? ((groupTotal2024 - groupTotal2023) / groupTotal2023 * 100).toFixed(1) : 'N/A';
