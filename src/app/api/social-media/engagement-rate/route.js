...
const buildWhereConditions = (filters) => {
	...
	if (filters.dateRange?.from) {
		conditions.push(
			sql`DATE(${mentionsClassifyPublic.inserttime}) >= ${filters.dateRange.from}`
		);
	}

	if (filters.dateRange?.to) {
		conditions.push(
			sql`DATE(${mentionsClassifyPublic.inserttime}) <= ${filters.dateRange.to}`
		);
	}
...
export async function GET(request) {
	try {
		...
		console.log(
			"📊 Engagement Rate Over Time API - Processing filters:",
			filters
		);
		...
		const baseConditions = [isNotNull(mentionsClassifyPublic.inserttime)];
		...
		try {
			...
			const engagementData = await db
				.select({
					date: sql`TO_CHAR(DATE(${mentionsClassifyPublic.inserttime}), 'YYYY-MM-DD')`.as(
						"date"
					),
					postCount: sql`COUNT(*)::int`.as("postCount"),
					avgEngagementRate:
						sql`COALESCE(AVG(${mentionsClassifyPublic.engagementrate}), 0)::float`.as(
							"avgEngagementRate"
						),
					totalReach:
						sql`COALESCE(SUM(${mentionsClassifyPublic.reach}), 0)::bigint`.as(
							"totalReach"
						),
					totalInteractions:
			...
			console.log(
				`✅ Fetched ${formattedData.length} engagement records from ${filters.dateRange.from} to ${filters.dateRange.to}`
			);
