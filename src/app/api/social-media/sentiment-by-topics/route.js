  ...
  try {
    ...
    try {
      ...
      const sentimentByTopics = await db
        ...
        .groupBy(mentionsClassifyPublic.topic, mentionsClassifyPublic.autosentiment)
        .orderBy(desc(count()));
      ...
      console.log('📊 Sentiment by Topics data processed:', processedData.length, 'topics');
      ...
      const response = {
        ...
        meta: {
          ...
          totalRecords: processedData.reduce((sum, topic) => sum + topic.total, 0)
        }
      };
      ...
      const fallbackData = [
        { topic: 'Technology', positive: 45, negative: 12, neutral: 23, total: 80 },
        { topic: 'Healthcare', positive: 38, negative: 8, neutral: 19, total: 65 },
        { topic: 'Education', positive: 32, negative: 15, neutral: 18, total: 65 },
        { topic: 'Environment', positive: 28, negative: 20, neutral: 12, total: 60 },
        { topic: 'Politics', positive: 15, negative: 35, neutral: 10, total: 60 }
      ];
      ...
      return NextResponse.json({
        ...
        meta: {
          ...
          totalRecords: fallbackData.reduce((sum, topic) => sum + topic.total, 0)
        }
      });
    }
