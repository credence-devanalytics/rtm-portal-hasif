    ...
    try {
      ...
      const authorStats = await db
        .select({
          ...
          followersCount: sql`MAX(COALESCE(${mentionsClassifyPublic.followerscount}, 0))`.as('followers_count'),
          totalPosts: sql`COUNT(*)`.as('total_posts'),
          positiveCount: sql`COUNT(CASE WHEN ${mentionsClassifyPublic.autosentiment} = 'positive' THEN 1 END)`.as('positive_count'),
          negativeCount: sql`COUNT(CASE WHEN ${mentionsClassifyPublic.autosentiment} = 'negative' THEN 1 END)`.as('negative_count'),
          neutralCount: sql`COUNT(CASE WHEN ${mentionsClassifyPublic.autosentiment} = 'neutral' THEN 1 END)`.as('neutral_count'),
          topicCount: sql`COUNT(DISTINCT ${mentionsClassifyPublic.topic})`.as('topic_count'),
        })
      ...
      const generateFallbackAuthor = (index) => {
        const topics = ['Politics', 'Health', 'Technology', 'Entertainment', 'Sports', 'Education'];
        const totalPosts = Math.floor(Math.random() * 50) + 10;
        const posCount = Math.floor(Math.random() * totalPosts * 0.6);
        const negCount = Math.floor(Math.random() * (totalPosts - posCount) * 0.4);
      ...
      const fallbackAuthors = Array.from({ length: Math.min(limit, 10) }, (_, index) => 
        generateFallbackAuthor(index)
      );
