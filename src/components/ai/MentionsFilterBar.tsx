...
export function MentionsFilterBar({
  ...
  const uniqueTopics = Array.from(new Set(availableTopics.filter(Boolean))).sort();

  // Update filters when values change
  const updateFilters = (newFilters: Partial<MentionsFilterBarProps["filters"]>) => {
    const updatedFilters = { ...filters, ...newFilters };
    // Remove undefined filter values to avoid API issues
    Object.keys(updatedFilters).forEach(key => {
      if (updatedFilters[key as keyof typeof updatedFilters] === 'all' ||
          ...
          <TabsContent value="filters" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  updateFilters({ sentiment: value === "all" ? undefined : value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
