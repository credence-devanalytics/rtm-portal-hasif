...
import { rtmklikGender, rtmklikAge } from "@/lib/schema";
import { eq, and, inArray } from "drizzle-orm";

export interface RTMKlikFilters {
	gender?: string | null;
...
export interface RTMKlikAnalysisResult {
	...
	filters: RTMKlikFilters;
}

// Analysis function for advertisement recommendations
export function generateRTMKlikAdvertisementAnalysis(
	data: any[],
	filters: RTMKlikFilters
): RTMKlikAnalysisResult {
	...
	const timeSlotAnalysis = data.map((record) => {
		...
		const targetAgeData = record.ageData.filter((a) =>
			ageBrackets.includes(a.age)
		);

		// Calculate total metrics for target demographic (use actual metric names)
		const totalUsers =
			(targetGender?.activeUsers || targetGender?.users || 0) +
			targetAgeData.reduce(
				(sum, age) => sum + (age.activeUsers || age.users || 0),
				0
			);
		const totalSessions =
			(targetGender?.screenPageViews || targetGender?.sessions || 0) +
			targetAgeData.reduce(
				(sum, age) => sum + (age.screenPageViews || age.sessions || 0),
				0
			);
	...
	// Generate insights
	const totalUsers = timeSlotAnalysis.reduce(
		(sum, slot) => sum + slot.totalUsers,
		0
	);
	const totalSessions = timeSlotAnalysis.reduce(
		(sum, slot) => sum + slot.totalSessions,
		0
	);
	...
	return {
		...
		insights: {
			...
			avgEngagementPerSlot:
				(totalUsers + totalSessions * 1.5) / timeSlotAnalysis.length,
		},
		filters,
	};
}

export async function fetchRTMKlikAudienceData(
	filters: RTMKlikFilters & { analysis?: boolean }
) {
	try {
		console.log("Fetching RTMKlik audience data with filters:", filters);
		...
		if (joinedData.length > 0) {
			...
			console.log("Unique metrics found:", [
				...new Set(
					joinedData
						.map((row) => row.genderMetric)
						.concat(joinedData.map((row) => row.ageMetric))
				),
			]);
		}
		...
		const groupedData = joinedData.reduce((acc: any, row) => {
			...
			let genderIndex = acc[key].genderData.findIndex(
				(g: any) => g.gender === row.gender
			);
			if (genderIndex === -1) {
			...
			let ageIndex = acc[key].ageData.findIndex((a: any) => a.age === row.age);
			if (ageIndex === -1) {
		...
		if (result.length > 0) {
			...
			console.log('Available ages:', [...new Set(result.flatMap((r: any) => r.ageData.map((a: any) => a.age)))]);
		}
		...
		if (
			...
			filteredData = result.filter((record: any) => {
				...
				if (filters.ageRange && filters.ageRange !== 'all') {
					...
					if (!ageMatch) {
						console.log(`Age filter mismatch. Looking for: ${filters.ageRange}, available:`, record.ageData.map((a: any) => a.age));
						return false;
					}
				}
		...
		if (filters.analysis) {
			const analysisResult = generateRTMKlikAdvertisementAnalysis(
				filteredData,
				filters
			);
		...
		console.error("RTMKlik audience data fetch error:", error);
		return {
			success: false,
